...
public class Minhasher {
	...
	public long hash(Set<String> doc) {
		...
		for (String word : doc) {
			// in principle there is no need to take the absolute value.
			// However, it looks a bit strange if the hash outcomes are negative
			// numbers.
			long permutedWordIndex = Math.abs(this.hf.hashString(word,
